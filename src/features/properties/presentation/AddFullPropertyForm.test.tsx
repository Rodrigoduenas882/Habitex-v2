    ...
    <QueryClientProvider client={client}>
      <MemoryRouter initialEntries={['/properties/new']}>
        <Routes>
          <Route
            path="/properties/new"
            element={<AddFullPropertyForm administrationId="admin-1" onBack={() => {}} />}
          />
...
describe('AddFullPropertyForm', () => {
  it('disables the submit button and shows the management-access reason when the subscription denies access', async () => {
    ...
    expect(screen.getByRole('button', { name: 'Guardar inmueble' })).toBeDisabled()
  })

  it('leaves the submit button enabled and shows no gate reason when the subscription grants access', async () => {
    renderForm()

    expect(await screen.findByRole('button', { name: 'Guardar inmueble' })).toBeEnabled()
    expect(
